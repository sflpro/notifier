...
public class AmazonSnsPushMessageSenderTest extends AbstractPushNotificationUnitTest {
    ...
    public void testSend() {
        ...
        when(amazonSnsApiCommunicator.sendPushNotification(isA(SendPushNotificationRequestMessageInformation.class),eq(message.destinationRouteToken()))).then(invocation -> {
            ...
            assertThat((String)invocation.getArgument(1)).isEqualTo(message.destinationRouteToken());
            return new SendPushNotificationResponse(messageId);
        });
        assertThat( pushMessageSender.send(message).messageId()).isEqualTo(messageId);
        verify(amazonSnsApiCommunicator).sendPushNotification(isA(SendPushNotificationRequestMessageInformation.class),eq(message.destinationRouteToken()));
    }
}
